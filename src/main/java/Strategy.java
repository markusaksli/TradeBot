public class Strategy {
    ...
    public static void update(Currency currency) {
        ...
        if (checkRSI(rsi) + checkMACD(lastMACD, tempMACD) >= 2) { //As we add more indicators we can use this to open a trade if we get a confluence of 2 or more
            System.out.println("---" + currency.getCoin() + " trade opened due to RSI of " + rsi + " and unclosed MACD histogram growing by " + Formatter.formatPercent((tempMACD - lastMACD) / lastMACD) + " in current candle");
            BuySell.open(currency, amount / currency.getPrice());
        }
    }
