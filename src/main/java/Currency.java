...
public class Currency {
    ...
    public double getPrice() { //Getting the current price using Binance API
        try {
            return (CurrentAPI.getBinanceApi().pricesMap().get(symbol.getSymbol())).doubleValue();
    ...
    public List<BinanceCandlestick> getKlines(int lenght) throws BinanceApiException {
        return (CurrentAPI.getBinanceApi()).klines(symbol, BinanceInterval.FIVE_MIN, lenght, null);
    }
}
